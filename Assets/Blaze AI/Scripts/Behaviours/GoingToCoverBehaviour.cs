...
namespace BlazeAISpace
{
    public class GoingToCoverBehaviour : MonoBehaviour
    {
        ...
        {   
            ...
            int hitReduction = 0;
            Transform playerCover = coverShooterBehaviour.GetTargetCover(blaze.enemyToAttack.transform.position);
            ...
            for (int i=0; i<hits; i++) {
                if (Vector3.Distance(blaze.ValidateYPoint(coverColls[i].transform.position), blaze.enemyToAttack.transform.position) + 2 >= coverShooterBehaviour.distanceFromEnemy || 
                    coverColls[i].bounds.size.y < minCoverHeight || coverColls[i].transform == coverToAvoid || coverColls[i].transform.IsChildOf(playerCover)) {
